...
/*   Updated: 2025/10/17 12:44:27 by luda-cun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	free_mini_map_image(t_context *ctx)
{
	if (ctx->image->white)
	{
		if (ctx->image->whiteptr)
			mlx_destroy_image(ctx->window->mlx, ctx->image->whiteptr);
		free(ctx->image->white);
	}
	if (ctx->image->red)
	{
		if (ctx->image->redptr)
			mlx_destroy_image(ctx->window->mlx, ctx->image->redptr);
		free(ctx->image->red);
	}
	if (ctx->image->black)
	{
		if (ctx->image->blackptr)
			mlx_destroy_image(ctx->window->mlx, ctx->image->blackptr);
		free(ctx->image->black);
	}
}
