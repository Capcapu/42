...
/*   Updated: 2025/10/17 12:37:52 by luda-cun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_tex_data_bis(t_tex *tex, t_context *ctx, int wall_o)
{
	if (wall_o == 3)
	{
		tex->data = ctx->tex->south->img_ptr + ((ctx->number_image / 2) % 15)
			* 512 * 2;
	...
	{
		tex->data = ctx->tex->north->img_ptr + ((ctx->number_image / 2) % 15)
			* 512 * 2;
...
void	fill_tex_data(t_tex *tex, t_context *ctx)
{
	...
	if (wall_o == 1)
	{
		tex->data = ctx->tex->east->img_ptr + ((ctx->number_image / 2) % 15)
			* 512 * 2;
	...
	{
		tex->data = ctx->tex->west->img_ptr + ((ctx->number_image / 2) % 15)
			* 512 * 2;
